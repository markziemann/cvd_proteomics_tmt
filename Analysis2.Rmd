...
# Overlap between Plex A and B Differentially Expressed Proteins Based On significance

```{r}
# Significant Proteins
sig_A <- results_A %>%
  dplyr::filter(adj.P.Val < 0.05) %>%
  dplyr::select(Protein, gene, logFC, adj.P.Val)

sig_B <- results_B %>%
  dplyr::filter(adj.P.Val < 0.05) %>%
  dplyr::select(Protein, gene, logFC, adj.P.Val)

#Shared proteins between plexes
shared_proteins <- intersect(sig_A$Protein, sig_B$Protein)

shared_table <- sig_A %>%
  dplyr::filter(Protein %in% shared_proteins) %>%
  dplyr::rename(logFC_A = logFC, adjP_A = adj.P.Val) %>%
  dplyr::left_join(
    sig_B %>%
      dplyr::rename(logFC_B = logFC, adjP_B = adj.P.Val),
    by = "Protein"
  )

# Plex A
unique_A <- setdiff(sig_A$Protein, sig_B$Protein)

sig_A_only <- sig_A %>%
  dplyr::filter(Protein %in% unique_A)

# Plex B
unique_B <- setdiff(sig_B$Protein, sig_A$Protein)

sig_B_only <- sig_B %>%
  dplyr::filter(Protein %in% unique_B)

cat("Significant proteins in Plex A:", nrow(sig_A), "\n")
cat("Significant proteins in Plex B:", nrow(sig_B), "\n")
cat("Shared significant proteins:", length(shared_proteins), "\n")
cat("Unique to Plex A:", length(unique_A), "\n")
cat("Unique to Plex B:", length(unique_B), "\n")

kable(shared_table, caption = "Shared Significant Proteins (Plex A vs Plex B)")
kable(sig_A_only, caption = "Plex A Only Significant Proteins")
kable(sig_B_only, caption = "Plex B Only Significant Proteins")
```

## Ranking of proteins by logFC
```{r}
# Plex A (up-regulated)
results_A <- as.data.frame(results_A)
results_B <- as.data.frame(results_B)

results_A$logFC <- as.numeric(results_A$logFC)
results_B$logFC <- as.numeric(results_B$logFC)

topUp_A <- results_A[order(results_A$logFC, decreasing = TRUE), ]
topUp_A <- topUp_A[1:50, ]

# Plex B (up-regulated)
topUp_B <- results_B[order(results_B$logFC, decreasing = TRUE), ]
topUp_B <- topUp_B[1:50, ]

# Plex A (down-regulated)
topDown_A <- results_A[order(results_A$logFC, decreasing = FALSE), ]
topDown_A <- topDown_A[1:50, ]

# Plex B (down-regulated)
topDown_B <- results_B[order(results_B$logFC, decreasing = FALSE), ]
topDown_B <- topDown_B[1:50, ]

intersect(topUp_A$Protein, topUp_B$Protein)
intersect(topDown_A$Protein, topDown_B$Protein)

# Rank consistency plot
common <- intersect(results_A$Protein, results_B$Protein)

rank_df <- data.frame(
  Protein = common,
  Rank_A = rank(-results_A$logFC[match(common, results_A$Protein)]),
  Rank_B = rank(-results_B$logFC[match(common, results_B$Protein)])
)

cor(rank_df$Rank_A, rank_df$Rank_B)
```
...
gene_list_A <- rank_A$t
names(gene_list_A) <- rank_A$gene
...
gene_list_B <- rank_B$t
names(gene_list_B) <- rank_B$gene

# Convert to MITCH input format
df_A <- data.frame(logFC = gene_list_A)
df_B <- data.frame(logFC = gene_list_B)

# Get gene sets (Reactome + GO)
...
go_sets <- split(msig_go$gene_symbol,
                 msig_go$gs_name)

# Mitch Plex A
mitch_A_reactome <- mitch_calc(
  df_A,
  reactome_sets,
  minsetsize = 5
)

res_A_reactome <- mitch_A_reactome$enrichment_result %>%
  arrange(p.adjustANOVA)

# Mitch plex B
mitch_B_reactome <- mitch_calc(
  df_B,
  reactome_sets,
  minsetsize = 5
)

res_B_reactome <- mitch_B_reactome$enrichment_result %>%
  arrange(p.adjustANOVA)

# Run mitch
mitch_A_go <- mitch_calc(
  df_A,
  go_sets,
  minsetsize = 5
)

res_A_go <- mitch_A_go$enrichment_result %>%
  arrange(p.adjustANOVA)

mitch_B_go <- mitch_calc(
  df_B,
  go_sets,
  minsetsize = 5
)

res_B_go <- mitch_B_go$enrichment_result %>%
  arrange(p.adjustANOVA)

# Extract significant pathways

sig_A_reactome <- res_A_reactome %>%
  filter(p.adjustANOVA < 0.05)

sig_B_reactome <- res_B_reactome %>%
  filter(p.adjustANOVA < 0.05)
...
length(intersect(names(gene_list_A), unlist(reactome_sets)))
length(intersect(names(gene_list_A), unlist(go_sets)))

head(res_A_reactome, 10)
head(res_B_reactome, 10)


# Common pathways in both plexes
# Plex A
A_table <- res_A_reactome %>%
  dplyr::select(set, setSize, pANOVA, p.adjustANOVA, s.dist) %>%
  rename_with(~ paste0("A_", .), -set)

# Plex B
B_table <- res_B_reactome %>%
  dplyr::select(set, setSize, pANOVA, p.adjustANOVA, s.dist) %>%
  rename_with(~ paste0("B_", .), -set)

# Merge common pathways
common_pathways_reactome <- inner_join(A_table, B_table, by = "set")

# Sort by significance
common_pathways_reactome <- common_pathways_reactome %>%
  arrange(A_p.adjustANOVA, B_p.adjustANOVA)

kable(common_pathways_reactome)
...
## Combined Plex Pathwya enrichment
```{r}
# combined Plex
